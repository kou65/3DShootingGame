...
bool Model::VertexBufferCreate(
	...
	// 頂点バッファ作成
	p_graphics->GetDevice()->CreateVertexBuffer(
		// 頂点バッファサイズ(CustomVertex * 頂点数)
		(sizeof(SkinCustomVertex) * total_vertex_num),
		// リソースの使用法
		0,
		// 柔軟な頂点フォーマットの型を指定する
		0,
	...
	// 頂点数分用意する
	SkinCustomVertex *custom_vertex_list;

	// ロック
	(*p_vertex_buffer)->Lock(
		0,
		total_vertex_num * sizeof(SkinCustomVertex),
