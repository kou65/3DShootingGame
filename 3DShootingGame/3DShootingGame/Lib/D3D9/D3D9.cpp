...
namespace D3D9 {
	...
	void SamplerStateUMirror() {
		...
		// 描画Uを反転させる
		d3d_device9->SetSamplerState(0, D3DSAMP_ADDRESSU, D3DTADDRESS_MIRROR);
	}


	void SamplerStateVMirror() {
		// 描画Vを反転させる
		d3d_device9->SetSamplerState(0, D3DSAMP_ADDRESSV, D3DTADDRESS_MIRROR);
	}
	...
	void SamplerStateClamp() {
		...
		// 描画UVループテクスチャ描画なし
		d3d_device9->SetSamplerState(0, D3DSAMP_ADDRESSU, D3DTADDRESS_CLAMP);
		d3d_device9->SetSamplerState(0, D3DSAMP_ADDRESSV, D3DTADDRESS_CLAMP);
	}
	...
	void SamplerStateWrap() {
		...
		// 描画UVループテクスチャ描画あり
		d3d_device9->SetSamplerState(0, D3DSAMP_ADDRESSU, D3DTADDRESS_WRAP);
		d3d_device9->SetSamplerState(0, D3DSAMP_ADDRESSV, D3DTADDRESS_WRAP);
	}


	void SamplerStateBorderColor() {
		d3d_device9->SetSamplerState(0, D3DSAMP_BORDERCOLOR, 0xffffff);
	}


	bool GetSamplerState(DWORD sampler_stage_index, D3DSAMPLERSTATETYPE member_type, DWORD *get_state) {

		if (d3d_device9->GetSamplerState(sampler_stage_index, member_type, get_state) != MB_OK) {
			return false;
		}
